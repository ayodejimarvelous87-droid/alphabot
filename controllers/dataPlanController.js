...
//
// Categories are unified across providers.
//
// Example:
//
//   OPLUG     -> SME
...
//   BlitzPay  -> SME
//
// All three appear under the same SME category.
//
// Provider plan objects themselves are NOT merged.
// Each provider plan remains its own record.
// ============================================================
...
allPlans.forEach(plan => {
  ...
  if(network.includes("mtn")){
    ...
    network.includes("9mobile") ||
    network.includes("etisalat")
  ){
    network = "9mobile";
  }
  else{
  ...
  // Keep unknown networks out of the customer catalogue.
  if(network === "Other"){
    return;
  }
  ...
  const category =
    String(formatCategory(plan) || "Standard")
      .trim();

  if(!grouped[network][category]){
    grouped[network][category] = [];
  }

  // IMPORTANT:
  // Do not merge, deduplicate, rename, or replace provider plans.
  // Just place the original plan record into its category.
  grouped[network][category].push(plan);

});


// Remove empty categories/networks only.
Object.keys(grouped).forEach(network => {

  Object.keys(grouped[network]).forEach(category => {

    if(
      !Array.isArray(grouped[network][category]) ||
      grouped[network][category].length === 0
    ){
  ...
  if(Object.keys(grouped[network]).length === 0){
