...
const { handleAdminWithdrawal } = require("./handlers/adminWithdrawalHandler");
...
const handleMessage = async (phone, message) => {
  ...
  const text = message.toLowerCase().trim();
    ...
    const command = menuMap[text] || text;
      ...
      const handledAdmin = await handleAdmin({
        phone,
        text,
        state,
        sendMessage,
  ...
  if (!state) {
    ...
    const handledAdminWithdrawal = await handleAdminWithdrawal({
      phone,
      text,
      sendMessage
    });

    if (handledAdminWithdrawal) return;
